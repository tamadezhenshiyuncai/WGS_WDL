...
task concat_vcf {
    ...
    output {
        ...
        File concat_vcf = "~{batch_path}/variant/~{batch_number}.wgs.vcf.gz"
        File concat_vcf_tbi = "~{batch_path}/variant/~{batch_number}.wgs.vcf.gz.tbi"
    }
}
...
task contamination {
    input {
        ...
        String batch_path
        String batch_number
        File concat_vcf  #来自于concat_vcf_2的结果
        File concat_vcf_tbi
        Int cpu
        Float? mem
    }
    command<<<
        ...
        ky_anno_python3 \
        -m bio_toolkit.pipeline.contamination \
        --sample_id ~{batch_number} \
        --vcf ~{concat_vcf} \
        --output ~{batch_path}/QC && \

        python3 ~{tools_dir}/csv2excel.py \
        --txt ~{batch_path}/QC/~{batch_number}-contamination.tsv \
        --outfile ~{batch_path}/QC/~{batch_number}-contamination.xlsx \
        --add_sample2
    >>>
    runtime {
        cpu: cpu
    ...
    output{
        File contamination_xls = "~{batch_path}/QC/~{batch_number}-contamination.xlsx"
    }
}
...
task variant_annotation {
    ...
    command <<<
        ...
        perl \
            ...
            ~{tools_dir}/wgs_database/pipeline_anno/bgi_anno/etc/config_WES.Roche.pl \
            ~{split_vcf} \
        ...
        ~{tools_dir}/tools/python3\
                ~{tools_dir}/add_af.py\
                --inn ~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG\
                --outf ~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD\
                --base ~{tools_dir}/wgs_database/project_doc/gnomAD_split/~{chr}_~{part_num}.sort\
                --type gnomAD && \

        ~{tools_dir}/tools/python3\
                ~{tools_dir}/add_af.py\
                --inn ~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD\
                --outf ~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse\
        ...
        ~{tools_dir}/wgs_database/pipeline_anno/anno2xlsx_v2/anno2xlsx\
                -snv ~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse \
                -wgs -acmg -gender $sex \
                -cfg ~{tools_dir}/etc/config.toml && \
        
        ~{tools_dir}/tools/py3-spliceai \
                ~{tools_dir}/spliceai_create.py \
                -b ~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse.WGS.xlsx \
                -p ~{batch_path}/annotation/part_anno \
                -o ~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse.WGS \
                -c ~{tools_dir}/spliceai_yantian.yaml \
                --sheet intron --out_format excel && \
        
        sh ~{batch_path}/annotation/part_anno/spliceai.~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse.WGS.sh && \
        ...
        ~{tools_dir}/tools/py3-spliceai \
                ~{tools_dir}/spliceai_create.py\
                -b ~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse.Tier1.xlsx\
                -p ~{batch_path}/annotation/part_anno\
                -o ~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse.Tier1\
                -c ~{tools_dir}/spliceai_yantian.yaml\
                --sheet filter_variants --out_format excel && \
        
        sh ~{batch_path}/annotation/part_anno/spliceai.~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse.Tier1.sh && \
    ...
    output {
        ...
        File gnomad = "~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD"
        File inhouse_wgs = "~{batch_path}/annotation/part_anno/~{chr}_~{part_num}.anno.anno.bed.ACMG.gnomAD.inhouse.WGS.xlsx"
    }
}
...
task cnvnator {
    ...
    output {
        File chr_cnv = "~{batch_path}/CNV/cnvnator/chr_all.CNV"
    }
}
...
task mops {    #依赖 bqsr_check、qc_collect
    ...
    command <<<
        ...
        ret=`python3 ~{tools_dir}/CNV/mops/sex_link.py ~{tools_dir}/wgs_database/CNV_database/mops_control/normal19/control ~{batch_path}/CNV/cnvnator/mops/~{chr} ~{batch_path}/QC/sex.txt ~{chr}`
        ...
        ln -sf ~{batch_path}/bam_chr/~{chr}.bqsr.bam ~{batch_path}/CNV/cnvnator/mops/~{chr}/sample.bam
        ln -sf ~{batch_path}/bam_chr/~{chr}.bqsr.bai ~{batch_path}/CNV/cnvnator/mops/~{chr}/sample.bai
        cd ~{batch_path}/CNV/cnvnator/mops/~{chr}
        Rscript-mops \
            ~{tools_dir}/CNV/mops/cnMOPS_cotrol.R \
            ~{chr} $ret ~{batch_path}/CNV/cnvnator/mops/~{chr} ~{batch_number}
    >>>
    runtime {
        cpu: cpu
        memory: "~{mem}GB"
...
task cnvnator_mops_combine {
    input {
        String tools_dir
        File cnvnator_results  #来自收集task cnvnator 的不同染色体的结果
